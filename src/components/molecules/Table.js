...
export default function Table({ onRefresh }) {
  ...
  const formatAuthors = (authors) => {
    ...
    return authors.map(author => author?.nama_penulis || 'Unknown').join(', ');
  };

  if (loading) {
    return (
      <div className="w-full p-8 text-center">
        <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-r-2 border-[#99A1AF]"></div>
  ...
  if (error) {
    return (
      <div className="w-full p-8 text-center">
        ...
        <button 
          onClick={fetchBooks}
          className="mt-2 px-4 py-2 bg-[#FB64B6] text-white rounded hover:bg-[#FF008A] transition-colors duration-300"
        >
          Coba Lagi
        </button>
      </div>
          ...
          {books.length === 0 ? (
            ...
            books.map((book, index) => (
              <tr key={book.id} className="border-t-2 border-[#334155]">
                <td className="py-3 px-4 font-medium">{book.judul}</td>
                <td className="py-3 px-4">{book.genre}</td>
                <td className="py-3 px-4">{formatAuthors(book.penulis)}</td>
                <td className="py-3 px-4">{book.penerbit}</td>
                <td className="py-3 px-4">{formatYear(book.tahun_terbit)}</td>
